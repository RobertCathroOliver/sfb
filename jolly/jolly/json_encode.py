...
def get_api_encoders(settings, api_property_encoders):
    ...
    api_encoders = {
        'Game' : (lambda game: {'type': 'game',
                                'href': get_uri('game', root_id=get_id(game)),
                                'title': game.title,
                                'map': {'type': 'map', 'href': get_uri('map', root_id=get_id(game))},
                                'players': [encode(p, api_property_encoders) for p in game.players],
                                'time': encode(game.current_time, api_property_encoders),
                                'log': {'type': 'log', 'href': get_uri('game-log', root_id=get_id(game))}}),
        'Player' : (lambda player: {'type': 'player',
                                    'href': get_uri('player', doc_id=get_id(player), root_id=get_id(player.game)),
                                    'name': player.name,
                                    ...
                                    'log': {'type': 'log', 'href': get_uri('log', doc_id=get_id(player), root_id=get_id(player.game))},
                                    'status': {'type': 'status', 'href': get_uri('status', doc_id=get_id(player), root_id=get_id(player.game))},
                                    'queue': {'type': 'queue', 'href': get_uri('command-queue',  doc_id=get_id(player), root_id=get_id(player.game))} }),
        'System' : (lambda system: {'type': 'unit' if system.is_unit() else 'system',
                                    'href': get_uri('unit', doc_id=get_id(system), root_id=get_id(system.owner.game)) if system.is_unit() else get_uri('system', doc_id=get_id(system), unit_id=get_id(system.owner), root_id=get_id(system.owner.owner.game)),
                                    'id': unicode(system.id),
                                    'owner': encode(system.owner, api_property_encoders),
                                    'prototype': {'type': 'system-prototype', 'href': get_uri('system-prototype', name=system.prototype.name) },
                                    ...
                                    'commands': [encode(c, api_property_encoders) for c in system.exposed_commands] }),
        'User' : (lambda user: {'type': 'user',
                                'href': get_uri('user', root_id=get_id(user)),
                                'name': user.name,
                                'email': user.email}),
        'Map' : (lambda _map: {'type': 'map',
                               'href': get_uri('map', root_id=get_id(_map.game)),
                               'game': encode(_map.game, api_property_encoders),
        ...
        'Status' : (lambda status: {'type': 'status',
                                    'href': get_uri('status', doc_id=get_id(status.owner), root_id=get_id(status.owner.game)),
                                    'status': status.status,
                                    'details': encode(status.details, api_property_encoders)}),
        'ActionLog' : (lambda log: {'type': 'log',
                                    'href': get_uri('game-log', root_id=get_id(log.owner)) if log.owner.__class__.__name__ is 'Game' else get_uri('log', doc_id=get_id(log.owner), root_id=get_id(log.owner.game)),
                                    'owner': encode(log.owner, api_property_encoders),
                                    'actions': encode(log.actions, api_property_encoders)}),
        'Prototype': (lambda prototype: {'type': 'system-prototype',
                                         'name': prototype.name,
                                         'commands': [encode(c, api_property_encoders) for c in prototype.exposed_commands]}),
        'CommandQueue': (lambda queue: {'type': 'queue',
                                        'href': get_uri('command-queue', doc_id=get_id(queue.owner), root_id=get_id(queue.owner.game)),
                                        'owner': encode(queue.owner, api_property_encoders),
                                        'commands': encode(queue.commands, api_property_encoders)}),
        'Command': (lambda command: {'type': 'command',
                                     'href': get_uri('command', doc_id=get_id(command), player_id=get_id(command.owner), root_id=get_id(command.owner.game)),
                                     'owner': encode(command.owner, api_property_encoders),
...
def get_api_property_encoders(settings):
    ...
    api_property_encoders.update({
        'Game' : (lambda game: {'type': 'game', 'title': game.title, 'href': get_uri('game', root_id=get_id(game))}),
        'Player' : (lambda player: {'type': 'player', 'name': player.name, 'href': get_uri('player', doc_id=get_id(player), root_id=get_id(player.game))}),
        'System' : (lambda system: encode(system, api_property_encoders, 'Unit') if system.is_unit() else {'type': 'system', 'id': unicode(system.id), 'href': get_uri('system', doc_id=get_id(system), unit_id=get_id(system.owner), root_id=get_id(system.owner.owner.game))}),
        'Unit' : (lambda unit: {'type': 'unit', 'id': unicode(unit.id), 'href': get_uri('unit', doc_id=get_id(unit), root_id=get_id(unit.owner.game))}),
        'User': (lambda user: {'type': 'user', 'name': user.name, 'href': get_uri('user', root_id=get_id(user))}),
        'Command': (lambda command: {'type': 'command',
                                     'href': get_uri('command', doc_id=get_id(command), player_id=get_id(command.owner), root_id=get_id(command.owner.game)),
                                     'time': encode(command.time, api_property_encoders),
                                     'template': encode(command.template, api_property_encoders)}),
        'Action': (lambda action: {'type': 'action',
        ...
        'CommandTemplate' : (lambda template: {'type': 'command-template',
                                               'name': template.name,
